package Snpcallerfaster_rerun;

## PROBLEMS: PROGRAM runs on zero files when tempfiles are deleted.

##############################
#
# Snpcallerfaster_rerun.pm
#
# written by Mark L. Farman
#
# Purpose: Read BLAST reports, count and report SNPs in uniquely aligned genomic segments 
#
# Note: uses the new string-based counting algorithm
#
##############################
...
# if no, write to alignment hash
  ...
  while(my $L = <BLAST>) {
    chomp($L);
    my @BLAST = split(/\t/, $L);
    my($qid, $sid, $qstart, $qend, $sstart, $send, $AlignSumm) = @BLAST;

    # Store query alignment information for the current HSP

    $QCounts{$qid} = "0" unless exists($QCounts{$qid});

    $QCounts{$qid} .= "0" x ($qend - length($QCounts{$qid})) if ($qend > length($QCounts{$qid}));        # Start a new count string if it doesn't exist. Zeroes indicate no alignment

    substr($QCounts{$qid}, $qstart - 1, 1 + $qend - $qstart) =~ tr/01/12/;                             # Increment counts in repeated regions

    # Store subject alignment information for the current HSP
    ...
    $SCounts{$sid} = "0" unless exists($SCounts{$sid});

    $SCounts{$sid} .= "0" x ($send - length($SCounts{$sid})) if ($send > length($SCounts{$sid}));        # Start a new count string if it doesn't exist. Zeroes indicate no alignment

    substr($SCounts{$sid}, $sstart - 1, 1 + $send - $sstart) =~ tr/01/12/;                             # Increment counts in repeated regions
...
sub COUNT_ALIGNED_MATCHES {
  ...
  for(my $m = 0; $m <= $Alignment - 1; $m ++) {                                             # loop thro$
    if(substr($QCounts{$qid}-1, $qpos, 1) eq 1 && substr($SCounts{$sid}, $spos-1, 1) eq 1) {
        $MAligned ++
    }
...
sub ANALYZE_ALIGNED_MISMATCHES {
  ...
  for(my $j = 0; $j <= $NumMutations-2; $j += 2) {			# Loop through mismatches, one pair of bases at a time
        ...
        if(($qnucl =~ /G|A|T|C/) && ($snucl =~ /G|A|T|C/)) {  		# check that both are simple nucleotide substitutions
            ...
            if(substr($QCounts{$qid}, $qpos-1, 1) == 1 && substr($SCounts{$sid}, $spos-1, 1) == 1) {    # make sure base positions are in uniquely aligned regions
            ...
            elsif(substr($QCounts{$qid}, $qpos-1, 1) == 1) {		# add comment on repeat status

                print OUT "$qid\t$sid\t$qpos\t$spos\t$qnucl\t$snucl\tSubject repeated"

            }

            elsif(substr($SCounts{$sid}, $spos-1, 1) == 1) {		# add comment on repeat status
